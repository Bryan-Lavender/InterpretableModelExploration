...
if __name__ == "__main__":
    ...
    for num_samp_k in [25,50,75,100,125,150,175,200,225]:
        config["sampling"]["sample_number"] = num_samp_k
        for i_episode in range(5):  # Run for a few episodes
            
            config["explanation_weights"]["model_path"] = tmpstr_modelPath + str(i_episode) + " n"+str(num_samp_k)
            config["explanation_output"]["MAE_MSE_RMSE_Rsq"] = tmpstr_MAE + str(i_episode)+ " n"+str(num_samp_k)
            start = time.time()
            ...
            initial_state = np.random.uniform(0, 0.1, size=(4,))
            ...
            np.save(config["explanation_output"]["uniform_sample_perc"] + str(i_episode)+ " n"+str(num_samp_k), np.array(avg_uniform_correctness))
            np.save(config["explanation_output"]["state_dist"] + str(i_episode)+ " n"+str(num_samp_k),np.array(state_diffs_dist))
            np.save(config["explanation_output"]["state_dist_vec"] + str(i_episode)+ " n"+str(num_samp_k),np.array(state_diffs_vec))

            np.save(config["explanation_output"]["policy_dist"]+ str(i_episode)+ " n"+str(num_samp_k), np.array(MainPolicyActionDiff))
            np.save(config["explanation_output"]["policy_perc"]+ str(i_episode)+ " n"+str(num_samp_k), np.array(avg_policy_correctness))
        np.save(config["explanation_output"]["time_saver"]+ " n"+str(num_samp_k), np.array(train_time))
    print("complete")
